...
public class Runner : MonoBehaviour {
	...
	[System.NonSerialized] public RunnerSpawner rs;
	...
	void Start() {
		...
		LeanTween.value(gameObject, 0.0f, 1.00f, 1.0f).
		setOnUpdate((float a) => {
			c = srOutline.color;
			c.a = a;
			srOutline.color = c;
		...
		.setOnComplete(() => {
			...
			LeanTween.value(gameObject, 1.0f, 0.00f, 1.0f).
	...
	private void OnTriggerStay2D(Collider2D collision) {
		if (isRunning) {
			if (collision.CompareTag("Player")) {
				...
				LeanTween.value(gameObject, 1.0f, 0.00f, 3.0f).
				setOnUpdate((float a) => {
					Color c = sr.color;
					c.a = a;
					sr.color = c;
